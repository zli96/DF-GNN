...
__global__ void
fused_forward_tb_kernel(int m, int nnz, int h, int f, const float *attn_row,
                        const float *attn_col, const int *row_ptr,
                        const int *col_ind, const float *in_feat,
                        const float negative_slope, const int *tile_scheduler,
                        float *edge_max, float *edge_sum, float *out_feat) {
...
gat_tb_forward_cuda(torch::Tensor attn_row, torch::Tensor attn_col,
  ...
  fused_forward_tb_kernel<<<dim3(tile_num, f, 1), dim3(32, h, 1)>>>(
      ...
      in_feat.data_ptr<float>(), negative_slope, tile_scheuler.data_ptr<int>(),
