...
void coo2csrKernel(int m, int nnz, int *cooRowInd, int *csrRowPtr) {
  cusparseHandle_t handle;
  checkCuSparseError(cusparseCreate(&handle));
  checkCuSparseError(cusparseXcoo2csr(handle, cooRowInd, nnz, m, csrRowPtr,
...
BucketPartHyb(int num_rows, int num_cols, torch::Tensor indptr,
  ...
  for (int i = 0; i < num_rows; ++i) {
    for (int j = indptr_data[i]; j < indptr_data[i + 1]; ++j) {
      ...
      int bucket_id =
          std::upper_bound(buckets_vec.begin(), buckets_vec.end(), degree - 1) -
          buckets_vec.begin();
      ...
      if (create_new_bucket) {
        // ICHECK(col_indices[part_id][bucket_id].size() % bucket_size == 0) <<
        // "Invalid padding";
  ...
  for (int part_id = 0; part_id < num_col_parts; ++part_id) {
    ...
    for (int bucket_id = 0; bucket_id < num_bkts; ++bucket_id) {
      ...
      // std::cout << "part&bucket " << part_id << " " << bucket_id <<
      // std::endl; std::cout << col_indices[part_id][bucket_id].size() << " "
      // << nnz * bucket_size << std::endl; std::cout <<
      // mask[part_id][bucket_id].size() << " " << nnz * bucket_size <<
      // std::endl;

      if (static_cast<int>(col_indices[part_id][bucket_id].size()) !=
          nnz * bucket_size)
        throw std::invalid_argument("Padding error.");
      if (static_cast<int>(mask[part_id][bucket_id].size()) !=
          nnz * bucket_size)
        throw std::invalid_argument("Padding error.");
      // ICHECK(static_cast<int>(col_indices[part_id][bucket_id].size()) == nnz
      // * bucket_size)
      //     << "Padding error.";
      // ICHECK(static_cast<int>(mask[part_id][bucket_id].size()) == nnz *
      // bucket_size)
      //     << "Padding error.";
      auto options =
          torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCPU);
      torch::Tensor row_indices_bucket_local = torch::empty({nnz}, options);
      torch::Tensor col_indices_bucket_local =
          torch::empty({nnz, bucket_size}, options);
      torch::Tensor mask_bucket_local =
          torch::empty({nnz, bucket_size}, options);
      if (nnz > 0) {
        auto opts =
            torch::TensorOptions().dtype(torch::kInt32).device(torch::kCPU);

        row_indices_bucket_local =
            torch::from_blob(row_indices[part_id][bucket_id].data(), {nnz},
                             opts)
                .clone();
        col_indices_bucket_local =
            torch::from_blob(col_indices[part_id][bucket_id].data(),
                             {nnz, bucket_size}, opts)
                .clone();
        mask_bucket_local = torch::from_blob(mask[part_id][bucket_id].data(),
                                             {nnz, bucket_size}, opts)
                                .clone();
      }
      row_indices_part_local.push_back(
          row_indices_bucket_local.to(torch::kLong));
      col_indices_part_local.push_back(
          col_indices_bucket_local.to(torch::kLong));
