...
def batch_graph_statistics(args):
    ...
    print(
        f"# of edges per subgraph mean {torch.mean(num_edges.float())} std {torch.std(num_edges.float())}"
    )
    # print(
...
def plot_batch_graph_perf(args):
    if args.conv == "gt":
...
def plot_full_graph_perf(args):
    ...
    print(args.dataset)
    for i, format in enumerate(formats):
        output = os.path.join(result_dir, f"{format}_dim{args.dim}_result.pkl")
        with open(output, "rb") as f:
            time_no_fuse, time_fuse = pickle.load(f)
        if i == 0:
            print(mean(time_no_fuse))
...
if __name__ == "__main__":
    ...
    # plot_batch_graph_perf(args)
    plot_full_graph_perf(args)
