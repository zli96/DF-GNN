...
from DFGNN.layers.gatconv_layer import GATConv
...
def load_dataset(args):
    ...
    # adjcent matrix of CSR format
    adj_csr = sp.csr_matrix(
        (numlist.numpy(), (row, col)), shape=(g.num_nodes(), g.num_nodes())
    )
    row_ptr = torch.from_numpy(adj_csr.indptr)
    col_ind = torch.from_numpy(adj_csr.indices)

    # adjcent matrix of CSC format
    adj_csr = sp.csr_matrix(
        (numlist.numpy(), col_ind.cpu().numpy(), row_ptr.cpu().numpy())
    )
    adj_csc = adj_csr.tocsc()
    permute = torch.from_numpy(adj_csc.data)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-heads", type=int, default=1, help="number of hidden attention heads"
    )
    parser.add_argument(
        "--negative-slope",
        type=float,
