...
class MoleculeDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name="Zinc", data_dir="./data/molecules"):
        t0 = time.time()
        self.name = name

        if self.name == "AqSol":
            self.num_atom_type = (
                65  # known meta-info about the AqSol dataset; can be calculated as well
            )
            self.num_bond_type = (
                5  # known meta-info about the AqSol dataset; can be calculated as well
            )
        else:
            self.num_atom_type = (
...
def positional_encoding(g, pos_enc_dim):
    ...
    n = g.number_of_nodes()
    if n <= pos_enc_dim:
        g.ndata["pos_enc"] = F.pad(
            g.ndata["pos_enc"], (0, pos_enc_dim - n + 1), value=float("0")
        )
...
class MoleculeDataset(torch.utils.data.Dataset):
    def __init__(self, name, data_dir="data/molecules/"):
        """
