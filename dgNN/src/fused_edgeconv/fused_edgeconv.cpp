...
std::vector<torch::Tensor> edgeconv_forward_cuda(const int k,
                                                 const torch::Tensor dst_ind,
                                                 const torch::Tensor h_src,
                                                 const torch::Tensor h_dst);

std::vector<torch::Tensor> edgeconv_forward(const int k,
...
torch::Tensor edgeconv_backward_cuda(const torch::Tensor grad_out,
                                     const torch::Tensor max_idx);

torch::Tensor edgeconv_backward(const torch::Tensor grad_out,
                                const torch::Tensor max_idx) {
  // printf("111111\n");
