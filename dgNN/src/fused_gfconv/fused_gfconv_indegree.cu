...
__global__ void fused_forward_kernel_indegree_hyper(
...
void gf_forward_indegree_hyper(int num_subgraph, int h, int f,
  ...
  cudaFuncSetAttribute(fused_forward_kernel_indegree_hyper<float>,
                       cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
  CUDA_KERNEL_CALL((fused_forward_kernel_indegree_hyper<float>), nblks, nthrs,
                   smem_size, h, f, nodes_subgraph, smem_nodes_subgraph,
                   store_node, store_flag, row, indptr, indices, val, Q, K, V,
                   out_feat);
}
