...
extern "C" bool isMul32(int x) { return (x > 0 && x % 32 == 0); }
...
__global__ void fused_forward_kernel_subgraph_mul32(
  ...
  // const int edge_lb = indptr[node_lb];     // the sum of edges in previous
  // subgraph
  const int num_nodes = node_hb - node_lb; // num of nodes in this subgraph
...
std::vector<torch::Tensor> gf_forward_cuda(torch::Tensor indptr,
  ...
  if (isMul32(f)) {
    ...
    gf_forward_no_multiple32(m, nnz, h, f, smem_consume, indptr.data_ptr<int>(),
                             indices.data_ptr<int>(), val.data_ptr<float>(),
                             Q.data_ptr<float>(), K.data_ptr<float>(),
                             V.data_ptr<float>(), out_feat.data_ptr<float>());
  }
...
std::vector<torch::Tensor> gf_outdegree_forward_cuda(
  ...
  if (isMul32(f)) {
    ...
    throw "gf_forward_outdegree not implented yet\n";
  }
