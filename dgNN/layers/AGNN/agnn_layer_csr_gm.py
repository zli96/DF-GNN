...
from dgNN.operators.fused_gtconv import GTConvFuse_inference_csr_gm
from dgNN.utils import benchmark

from .agnn_layer import AGNNConvDGL


class AGNNConv_csr_gm(AGNNConvDGL):
    def conv(self, H, indptr, indices, val):
        H_norm = F.normalize(H, p=2, dim=-1)
        out = GTConvFuse_inference_csr_gm(
            indptr,
            indices,
            val,
            H_norm,
