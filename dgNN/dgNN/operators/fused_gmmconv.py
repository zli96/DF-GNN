...
def GmmConvFuse(rowptr, colind, colptr, rowind, permute, node_feat, pseudo, mu, sigma):
    return GmmFuseFunction.apply(
        rowptr, colind, colptr, rowind, permute, node_feat, pseudo, mu, sigma
    )
...
class GmmFuseFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, rowptr, colind, colptr, rowind, permute, node_feat, pseudo, mu, sigma
    ):
