...
def main(args):
    ...
    edge_idx=edge_idx.to(args.gpu).long()
    ...
    model=Net(edge_idx,args.n_layers,n_feats,args.n_hidden,n_classes,heads,attn_drop=args.attn_drop,feat_drop=args.dropout,negative_slope=args.negative_slope).to(args.gpu)
    ...
    if args.output!=None:
        with open("{}".format(args.output),'a') as f:
            print("train_GAT_pyg,{} heads={} hidden_dim={},{:f}s,{:f}s,{}MB,{}".format(args.dataset,args.n_heads,args.n_hidden,train_time,inference_time,maxMemory,acc),file=f)
