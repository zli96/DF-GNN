...
class Model(nn.Module):
    def __init__(
        self, k, feature_dims, emb_dims, output_classes, input_dims=3, dropout_prob=0.5
    ):
        ...
        for i in range(self.num_layers):
            self.conv.append(
                EdgeConv(feature_dims[i - 1] if i > 0 else input_dims, feature_dims[i])
            )
...
if args.output != None:
    with open("{}".format(args.output), "a") as f:
        print(
            f"train_edgeconv_dgnn,{args.batch_size} {args.k},{train_time}s,{inference_time}s,{maxMemory}MB,{acc}",
            file=f,
        )
