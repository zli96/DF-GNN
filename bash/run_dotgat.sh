    ...
    for dataset in ${datasets[@]}; do
        for format in ${formats[@]}; do
            ...
            for bs in ${batch_sizes[@]}; do
                if [ -n "${test_flag}" ]; then
                    # # run with nolog
                    python -u dgNN/script/test/test_dotgat.py --dim $dim --batch-size $bs --data-dir ${data_dir} --dataset ${dataset} --format ${format} --conv dotgat
                else
                    python -u dgNN/script/test/test_dotgat.py --dim $dim --batch-size $bs --data-dir ${data_dir} --dataset ${dataset} --format ${format} --conv dotgat --store-result 2>&1 | tee -a log/day_${day}/${name}.log
